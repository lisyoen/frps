...
### 미완료 작업
- ❌ 집 miniPC에 FRP 서버(frps) 설치 안 됨
- ⚠️ 회사 frpc가 miniPC frps에 연결할 수 없음 (서버가 없어서)
...
- **설정 파일**: `configs/frps.toml`
- **설치 스크립트**: `scripts/install-frps.sh`
- **테스트 스크립트**: `scripts/test-frp.sh`, `scripts/test-llm-api.sh`
...
3. **Git 커밋**
   ```bash
   git add .github/
   git commit -m "세션 session-20251111-003 완료: miniPC에 FRP 서버 설치"
   git push
   ```

4. **회사 Spark 재연결 테스트**
   - 회사로 돌아가서 frpc 재연결 테스트
   - session-20251111-002 세션 재개

## 체크리스트

- [ ] 저장소 클론 또는 업데이트
- [ ] configs/frps.toml 설정 확인
- [ ] scripts/install-frps.sh 실행
- [ ] frps 서비스 정상 실행 확인
- [ ] 포트 7000, 8081 리스닝 확인
- [ ] 방화벽 설정 (필요 시)
- [ ] 라우터 포트 포워딩 설정
